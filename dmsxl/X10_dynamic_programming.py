...
### 遍历顺序
- **物品和背包容量可以互换顺序**  
  因为二维数组完整保留了每一层的状态，遍历顺序不影响状态转移的依赖关系。
...
- **遍历顺序**  
  **必须外层物品、内层背包容量**，不可互换。若交换顺序，无法保证每个物品只被选取一次。
  
- **遍历方向**  
  **内层背包容量必须逆序（从大到小）**，防止覆盖当前层状态，避免重复选取。
"""
# *** 正确写法（01背包）
dp = [0] * (W + 1)
...
- **遍历顺序**  
  ...
  - 外层物品 + 内层容量正序  
  - 外层容量正序 + 内层物品  
  （但如果是**组合数/排列数问题**，顺序会影响结果）

- **遍历方向**  
  **内层背包容量必须正序（从小到大）**，允许重复选取同一物品。
"""
...
"""
## 总结表
| 类型       | 数据结构 | 遍历顺序       | 遍历方向       | 可否互换顺序 |
|------------|----------|----------------|----------------|--------------|
| 01背包     | 二维数组 | 物品或容量优先 | 正序从小到大       | 可           |
| 01背包     | 一维数组 | 物品优先       | 容量逆序       | 不可  交换顺序或正序会导致重复选取        |
| 完全背包   | 二维数组 | 物品或容量优先 | 正序从小到大       | 可           |
| 完全背包   | 一维数组 | 物品或容量优先 | 容量正序       | 可（价值问题）|
| 组合数	 | 一维数组	| 物品优先	    | 正序	        | 外层物品保证组合唯一
| 排列数	 | 一维数组	| 容量优先	    | 正序	        | 外层容量考虑顺序

## 关键原因
- **01背包一维必须逆序**：防止覆盖未更新的状态，确保物品只选一次。  
- **完全背包一维必须正序**：允许利用当前层状态，支持多次选取。  
- **二维数组顺序无关**：状态独立存储，顺序不影响依赖关系。  

> **注意**：完全背包若涉及组合数/排列数问题，遍历顺序会影响结果：
> - **外层物品优先** → 组合数（不考虑顺序）  
> - **外层容量优先** → 排列数（考虑顺序）  
> 但如果是求最大价值，顺序不影响结果。
===================================================================================================
...
# affect the results in 0/1 Knapsack and Unbounded Knapsack (Complete Knapsack) problems.

# Combination vs Permutation Counting (Unbounded Knapsack Variants)
# (A) Combination Count (Order Doesn't Matter)
# Problem: How many ways to make W=4 using w=[1,2,3] (reuse allowed, [1,2] and [2,1] count as 1)?
dp = [0] * (W + 1)
dp[0] = 1  # Base case: 1 way to make 0

for i in range(n):  # Outer: items
    for j in range(w[i], W + 1):  # Inner: capacity
        dp[j] += dp[j - w[i]]

print(dp[W])  # Output: 4 ([1,1,1,1], [1,1,2], [1,3], [2,2])
# Why outer items?
    # Fixes item order (e.g., [1,2] is allowed, but [2,1] is not considered new).


# Permutation Count (Order Matters)
# Problem: How many ways to make W=4 where [1,2] and [2,1] count as 2?
dp = [0] * (W + 1)
dp[0] = 1

for j in range(W + 1):  # Outer: capacity
    for i in range(n):  # Inner: items
        if w[i] <= j:
            dp[j] += dp[j - w[i]]

print(dp[W])  # Output: 7 ([1,1,1,1], [1,1,2], [1,2,1], [2,1,1], [2,2], [1,3], [3,1])
# Why outer capacity?
    # Allows different orderings (e.g., [1,2] and [2,1] are both counted).

# 实际应用场景
# 组合数：
    # 硬币找零（[1,2] 和 [2,1] 算同一种找零方式）。
    # 背包问题的方案计数（只关心物品集合，不关心顺序）。

# 排列数：
    # 爬楼梯问题（每次走 1 或 2 步，[1,2] 和 [2,1] 是不同路径）。
    # 字符串的排列组合（顺序不同则结果不同）。
...
| 类型   | 物品使用规则               | 典型问题场景               |
|--------|---------------------------|---------------------------|
| 01背包 | 每个物品**最多选一次**     | 选/不选问题（如装最大价值）|
| 完全背包 | 每个物品**无限次使用**    | 无限兑换问题（如硬币找零）|


特性	01背包	完全背包
物品使用次数	每个物品只能选1次	每个物品可无限次选
容量遍历方向	必须倒序（从大到小）	必须正序（从小到大）
循环顺序灵活性	❌ 不可交换嵌套顺序	✅ 可交换（但推荐先物品）
状态转移依赖	依赖未选当前物品的状态	依赖可能已选当前物品的状态
空间优化必要性	必须倒序才能优化	正序即支持优化
...
01背包
    二维dp数组在遍历顺序上
        外层遍历物品,内层遍历背包容量 和 外层遍历背包容量,内层遍历物品 都是可以的!
        二维dp遍历的时候, 背包容量是从小到大. 且第二层for循环是从小到大遍历。
    一维dp数组
        物品遍历的for循环放在外层, 遍历背包的for循环放在内层, 且内层for循环倒序遍历!
        一维dp遍历的时候, 背包是从大到小。 第二层for循环是从大到小遍历。
...
一维dp关键理解
    ...
    正序遍历:
        dp[j] 会依赖 可能已被当前物品更新过的 dp[j - weight[i]], ***导致重复计算*** ???
    倒序遍历:
        dp[j] 依赖的是 未被当前物品污染的上一个状态，保证物品只使用一次。
...
"""
🔥 关键区别 (与01背包对比)
问题类型	循环顺序要求	容量遍历方向	能否交换循环顺序
01背包	必须外层物品，内层容量	倒序	❌ 不可交换
完全背包	外层物品或容量均可	正序	✅ 可交换
...
| 问题类型 | 循环顺序                  | 典型应用场景              |
|----------|--------------------------|-------------------------|
| 组合数   | **外层物品** + 内层容量  | 不考虑顺序的选取（如硬币组合） | 固定物品顺序，避免重复计算不同顺序的组合
| 排列数   | **外层容量** + 内层物品  | 考虑顺序的排列（如爬楼梯方式） | 对每个容量 j, 尝试所有可能的物品, 这样不同顺序的组合会被分别计算

本质原理
循环顺序	计算顺序	数学意义
物品→容量	按物品顺序逐步构建组合	组合数的动态规划
容量→物品	对每个容量考虑所有可能性	排列数的动态规划

循环顺序	计算方式	是否考虑顺序	适用问题
外层物品 → 内层容量	按物品顺序计算组合	❌ 不考虑顺序	零钱兑换 II (组合数)
外层容量 → 内层物品	对每个容量尝试所有物品	✅ 考虑顺序	组合总和 IV (排列数)
